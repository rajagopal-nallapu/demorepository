...
class User extends Controller implements IUser
{
    ...
    public function _updateRecurly($hidden,$data){
        try {
            $userInfoForRecurly = [
                'firstName'     => $data['first_name'],
                'lastName'      => $data['last_name'],
                'email'         => $data['email'],
                'recurly-token' => $hidden['recurly-token'],
                'recurly_account_code' => $data['recurly_account_code']
            ];
    ...
    public function addBillingInformation()
    {
        ...
        // print_r($_POST);
        // $this->getRecurlyResponse($_POST,$userData);
        // die;
        $recurlyResponse = $this->_createRecurly($_POST, $userData);
        print_r($recurlyResponse);die;
            ...
            if (count($recurlyResponse)>=2 && !empty($recurlyResponse)) {
                ...
                if ($this->RegistrationID) {
                    ...
                    if ($this->userID) {
                        ...
                        $error = "USER ACCESS REGISTRATION IS FAILED";
                        $this->messageFailedStatus($error,$userData);
                    }
                } else {
                    $error = "COMPANY REGISTRATION IS FAILED";
                    $this->messageFailedStatus($error,$userData);
                }
            } else {
                /*$failedURL ="";
                $zapierResponse = $this->createZapierRequest();
                $this->userModel->updateSignupLog($zapierLog); */
                $error = "CF - RECURLY ACCOUNT SUBSCRIPTION NOT FOUND";
                $this->messageFailedStatus($error,$userData);
                
            }    
        } catch(\Exception $error) {
            $error = " Problem with singup";
            $this->messageFailedStatus($error,$userData);
        }  
    }
